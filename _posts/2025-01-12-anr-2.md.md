...
## ANR分析方法
  ...
  2. *分析trace文件*，通过adb命令我们可以拿到发生ANR时各个线程的信息
     ...
     像上面这段trace表明在执行跨进程方法`doSomething`的时候发生了ANR，一般情况下我们可以通过主线程定位到问题所在，特殊的比如说CPU负载过高的情况和垃圾回收的情况找不到有效信息，其中CPU负载过高的情况下Main线程显示在等待Handler消息，垃圾回收的情况一般表现为找不到trace信息
